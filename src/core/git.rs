...
fn is_owned_artifact(path: &str) -> bool {
    path == "autoresearch-results"
        || path.starts_with("autoresearch-results/")
        || path == ".codex-autoresearch"
...
mod tests {
    ...
    fn test_is_owned_artifact() {
        ...
        assert!(!is_owned_artifact("src/state.json"));
        assert!(!is_owned_artifact("logs/runtime.log"));
        assert!(!is_owned_artifact("results.tsv"));
