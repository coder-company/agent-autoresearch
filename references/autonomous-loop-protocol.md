...
- Background calls `autoresearch runtime start --cwd <workspace_root>` so the confirmed launch manifest and detached runtime files are created in one binary-level handoff. `runtime start` first runs the native health preflight and requires canonical `context.json`; it blocks before writing launch artifacts if there are blockers or missing context. Use `--dry-run` only for tests or operator preflight. Do not create or update official Codex goals for background runs.
