...
It first runs the native health preflight and blocks without writing launch artifacts when blockers are present.
...
During session resume, `autoresearch resume ...` reconstructs the retained state from the TSV and compares it with `autoresearch-results/state.json`. Any mismatch triggers a mini-wizard rather than a silent full resume.
