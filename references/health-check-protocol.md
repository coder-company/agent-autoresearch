...
| Git state | For single-repo runs, `git status --porcelain` shows only expected files and autoresearch-owned artifacts, HEAD is attached to a branch, and no stale git lock files are present. For multi-repo runs, apply the same check to the primary repo and every companion repo declared in the launch manifest. | Warning if unexpected files; hard blocker if repo is corrupt, HEAD is detached, or git lock files are present |
