...
4. use `session-resume.md` and `interaction-wizard.md` for every new interactive launch (except `exec` mode),
...
8. write their documented logs and output files (for foreground iterating modes this means `autoresearch-results/results.tsv` and `autoresearch-results/state.json` as the core persistent artifacts; lessons remain binary-derived secondary output, and exec persists the TSV plus inactive canonical context metadata while cleaning up scratch JSON state before exit),
