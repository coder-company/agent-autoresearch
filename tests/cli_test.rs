...
#[test]
fn test_fix_iterations_are_recorded() {
    let dir = TempDir::new().unwrap();
    let output_dir = dir.path().join("autoresearch-results/fix/custom-budget");
    ...
    cmd()
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("\"iteration_budget\":7"));

    let summary = std::fs::read_to_string(output_dir.join("summary.md")).unwrap();
    let handoff = std::fs::read_to_string(output_dir.join("handoff.json")).unwrap();

    assert!(summary.contains("- Iteration budget: 7"));
    assert!(handoff.contains("\"iteration_budget\": 7"));
}
...
#[test]
fn test_evals_file_flag_reads_tsv() {
    let dir = TempDir::new().unwrap();
    let tsv_path = dir.path().join("results.tsv");
