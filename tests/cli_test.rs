...
fn test_handoff_defaults_to_repo_root_results_from_subdir() {
    ...
    cmd()
        .args([
            ...
            r#"[{"title":"fixed"}]"#,
            "--config",
            r#"{"goal":"fix login","scope":["src/auth/**"],"metric":"bug count","direction":"lower","verify":"cargo test","guard":"cargo fmt -- --check","verify_format":"metrics_json","primary_metric_key":"bugs","iterations":12,"stop_condition":"all auth bugs fixed","hypothesis_queue":["check auth"],"summary":{"risk":"low"}}"#,
            "--chain",
            "scenario,fix",
            "--evals",
            "--evals-interval",
